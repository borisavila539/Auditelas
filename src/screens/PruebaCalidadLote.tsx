  ...
  const getRollos = async () => {
    if (!cargando) {
      if (Importacion != '' || Lote != '') {
        setCargando(true)
        try {
          await reqResApiFinanza.get<RolloImporteLoteInterface[]>('Auditelas/getRolloImportacionLote/' + Importacion + '/' + (Lote != '' ? Lote : '-'))
            .then(resp => {
              setRollos(resp.data)
      ...
      <View style={{ width: '90%', maxWidth: 500, marginTop: 10 }}>
        <View style={{ flexDirection: 'row', width: '100%' }}>
          ...
          <View style={{ flexDirection: 'column', width: '50%' }}>
            ...
            <TextInput style={styles.textInputs} value={Lote}
